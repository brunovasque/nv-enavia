...
- **Contexto:** chamada dentro do handler `/propose`. O payload usa `executor_action: "propose"` com `mode: "enavia_propose"`, que é roteado para `/audit` do executor (COMPAT documentado no executor source).
...
- **Patch:** data atualizada para `"2026-04-26T00:00:00Z"` + comment explicando que é atualização manual.
...
- Arquivo alterado: `nv-enavia.js` — 2 patches pontuais (URL + BUILD marker)
